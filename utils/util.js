...
const getProtocolFeeStruct = (maxFee, minFee, maxRange, minRange) => {
  const result = regression.linear([
    [maxFee, minRange * 10000],
    [minFee, maxRange * 10000],
  ]);
  const gradient = result.equation[0];
  const yIntercept = result.equation[1];

  return {
    maxRange: toWei(maxRange + ""),
    minRange: toWei(minRange + ""),
    maxFee: maxFee,
    minFee: minFee,
    slope: Math.round(toWei(-gradient + "")) / 10000,
    constant: toWei(yIntercept + "") / 10000,
  };
};
