...
contract("OasisXExchange", (accounts) => {
  ...
  let deploy_core_contracts = async () => {
    let [registry, atomicizer] = await Promise.all([
      OasisXRegistry.new(),
      OasisXAtomicizer.new(),
    ]);
    let [exchange, statici] = await Promise.all([
      OasisXExchange.new([registry.address], accounts[0], [
        maxRange,
        minRange,
        100,
        200,
      ...
      OasisXStatic.new(atomicizer.address),
    ]);
    await registry.grantInitialExchangeAuthentication(exchange.address);
  ...
  it("matches erc1155 nft-nft swap order", async () => {
    ...
    const firstData = erc1155c.methods
      ...
      .encodeABI();
    const secondData = erc1155c.methods
      .safeTransferFrom(
        account_b,
        account_a,
        nfts[1].tokenId,
  ...
  it("matches erc1155 + erc20 <> erc1155 orders, matched left, real static call", async () => {
    ...
    const firstERC20Call = erc20c.methods
      ...
      .encodeABI();
    const firstERC1155Call = erc1155c.methods
    ...
    const secondERC1155Call = erc1155c.methods
