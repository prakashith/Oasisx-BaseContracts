...
contract("OasisXRegistry", (accounts) => {
  it("is deployed", async () => {
    return await OasisXRegistry.deployed();
  });

  it("does not allow additional grant", async () => {
    let registry = await OasisXRegistry.deployed();
    let tx = registry.grantInitialExchangeAuthentication(registry.address);
    return assertIsRejected(
      registry.grantInitialExchangeAuthentication(registry.address),
  ...
  it("does not allow proxy update from another account", async () => {
    ...
    let contract = new web3.eth.Contract(OwnableDelegateProxy.abi, proxy);
    return assertIsRejected(
      contract.methods.upgradeTo(registry.address).send({ from: accounts[0] }),
      /Returned error: VM Exception while processing transaction: revert/,
